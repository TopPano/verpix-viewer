...
export default class PanophotoPlayer {
  constructor(params) {
    ...
    this.loopBrand = params.loopBrand;
      ...
      if (this.isAutoplayEnabled) {
        if (this.autoPlay.accumulativeMovement > AUTO_PLAY.MANUAL_TO_AUTO_MOVEMENT_THRESHOLD) {
          ...
          if ((now() - this.autoPlay.startWaitTime) >= this.manualToAutoTime) {
            ...
            this.autoPlay.accumulativeMovement = 0;
            this.tip.show();
            if (this.brandContext.loopBrand) {
              // If loopBrand is true, show the brand in auto mode.
              this.brandContext.instance.show();
              this.dimScene();
            } else {
              if (this.brand.isShown) {
                // In some cases, the started brand is shown when we are changing
                // from manual to auto mode. We should hide the brand if loopBrand is false.
